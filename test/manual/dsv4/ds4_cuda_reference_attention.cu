...
void ds4_cuda_launch_v51_warp_softmax_direct_prepared_wmma_partial(
    dim3 grid,
    dim3 block,
    cudaStream_t stream,
    bool profile_stages,
...
void ds4_cuda_launch_v52_rowgroup_k_direct_prepared_wmma_partial(
    dim3 grid,
    dim3 block,
    cudaStream_t stream,
    bool profile_stages,
...
void ds4_cuda_launch_v53_warp_softmax_rowgroup_k_direct_prepared_wmma_partial(
    dim3 grid,
    dim3 block,
    cudaStream_t stream,
    bool profile_stages,
...
void ds4_cuda_launch_v54_staged_k_warp_softmax_rowgroup_direct_prepared_wmma_partial(
    dim3 grid,
    dim3 block,
    cudaStream_t stream,
    bool profile_stages,
...
void ds4_cuda_launch_v55_dimsplit_qk_warp_softmax_rowgroup_direct_prepared_wmma_partial(
    dim3 grid,
    dim3 block,
    cudaStream_t stream,
    bool profile_stages,
...
#define DSV4_DECLARE_DIRECT_PREPARED_LAUNCH(NAME)                         \
  void NAME(                                                              \
      dim3 grid,                                                          \
      dim3 block,                                                         \
      cudaStream_t stream,                                                \
...
constexpr int kV56CommonSwaWidth = 128;
constexpr int kV56CommonExtraWidth = 512;
constexpr int kV56CommonSwaPageSize = 256;
constexpr int kV56CommonExtraPageSize = 64;
constexpr int kV56CommonRowStride = 584;
constexpr int kV44ScoreWarpsPerBlock = 2;
constexpr int kV44ScoreRowsPerBlock = kScoreTileN * kV44ScoreWarpsPerBlock;
constexpr int kV44PreparedKTileElems = kScoreTileK * kV44ScoreRowsPerBlock;
constexpr int kV44PreparedVTileElems = kV44ScoreRowsPerBlock * kScoreTileK;
...
constexpr int kV47DimRoundGroups = kV31DimRoundGroups;
constexpr int kV47WarpsPerBlock = kV47DimRoundGroups * kV11DimGroups;
constexpr int kV47Threads = 32 * kV47WarpsPerBlock;
constexpr int kV47DimOuterRounds = kV11DimRounds / kV47DimRoundGroups;
static_assert(
    kV47WarpsPerBlock == kV47DimRoundGroups * kV11DimGroups,
    "v47 expects warps per block to divide into dim groups");
static_assert(
    kV11DimRounds % kV47DimRoundGroups == 0,
    "v47 expects dimension rounds to divide evenly into half-PV rounds");
constexpr int kV48HeadTilesPerBlock = kV28HeadTilesPerBlock;
constexpr int kV48Threads = kV8Threads;
...
constexpr int kV36DimChunk = 128;
constexpr int kV36DimChunks = kHeadDim / kV36DimChunk;
constexpr int kV36DimTileCount = kV36DimChunk / kScoreTileN;
constexpr int kV36AccElems = kScoreTileM * kV36DimChunk;
constexpr int kV36Threads = kV7Threads;
static_assert(
    kHeadDim % kV36DimChunk == 0,
    "v36 expects the head dimension to divide into chunks");
static_assert(
    kV36DimChunk % kScoreTileN == 0,
    "v36 expects dim chunks to divide into 16-dim WMMA tiles");
constexpr int kV37DimTileCount = kHeadDim / kScoreTileN;
constexpr int kV37DimTilesPerRound = 20;
constexpr int kV37AccElems = kV7AccElems;
constexpr int kV37Threads = 32 * kV37DimTilesPerRound;
static_assert(
    kHeadDim % kScoreTileN == 0,
    "v37 expects the head dimension to divide into WMMA dim tiles");
static_assert(
    kV37Threads <= 1024,
    "v37 expects the P@V dim-tile wave to fit in a CUDA block");
static_assert(
    kV37DimTilesPerRound > kV11WarpsPerBlock,
    "v37 should expose more P@V dim tiles per round than v25");
...
const char* attention_variant_name(AttentionVariant variant) {
  switch (variant) {
    ...
    case AttentionVariant::kOptimizedV35:
      return "v35";
    case AttentionVariant::kOptimizedV36:
      return "v36";
    case AttentionVariant::kOptimizedV37:
      return "v37";
    case AttentionVariant::kOptimizedV38:
      return "v38";
    case AttentionVariant::kOptimizedV39:
      return "v39";
    case AttentionVariant::kOptimizedV40:
      return "v40";
    case AttentionVariant::kOptimizedV41:
      return "v41";
    case AttentionVariant::kOptimizedV42:
      return "v42";
    case AttentionVariant::kOptimizedV43:
      return "v43";
    case AttentionVariant::kOptimizedV44:
      return "v44";
    ...
    case AttentionVariant::kOptimizedV50:
      return "v50";
    case AttentionVariant::kOptimizedV51:
      return "v51";
    case AttentionVariant::kOptimizedV52:
      return "v52";
    case AttentionVariant::kOptimizedV53:
      return "v53";
    case AttentionVariant::kOptimizedV54:
      return "v54";
    case AttentionVariant::kOptimizedV55:
      return "v55";
    case AttentionVariant::kOptimizedV56:
      return "v56";
    case AttentionVariant::kOptimizedV57:
      return "v57";
    case AttentionVariant::kOptimizedV58A:
      return "v58a";
...
__device__ __forceinline__ const __nv_bfloat16* ldmatrix_16x16_rowmajor_ptr(
    const __nv_bfloat16* tile,
    int lane) {
  const int matrix = lane >> 3;
  const int row = (lane & 7) + ((matrix & 1) ? 8 : 0);
  const int col = (matrix >= 2) ? 8 : 0;
  return tile + row * kScoreTileN + col;
}

__device__ __forceinline__ void ldmatrix_m8n8x4_b16(
    const __nv_bfloat16* ptr,
    uint32_t (&regs)[4]) {
  const uint32_t addr = shared_u32_addr(ptr);
  asm volatile(
...
__device__ __forceinline__ void ldmatrix_m8n8x4_trans_b16(
    const __nv_bfloat16* ptr,
    uint32_t (&regs)[4]) {
  const uint32_t addr = shared_u32_addr(ptr);
  asm volatile(
...
__device__ __forceinline__ __nv_bfloat16 scaled_fp8_e4m3fn_to_bfloat16(
    uint8_t bits,
    uint8_t scale_byte) {
  const uint16_t sign = static_cast<uint16_t>(bits & 0x80) << 8;
  const int exponent = (bits >> 3) & 0x0f;
  const int mantissa = bits & 0x07;
  if (exponent == 0 && mantissa == 0) {
    return __ushort_as_bfloat16(static_cast<unsigned short>(sign));
  }

  int out_exp = 0;
  int out_mantissa = 0;
  if (exponent == 0) {
    const int leading_bit = mantissa >= 4 ? 2 : (mantissa >= 2 ? 1 : 0);
    out_exp = static_cast<int>(scale_byte) + leading_bit - 9;
    out_mantissa = (mantissa - (1 << leading_bit)) << (7 - leading_bit);
  } else {
    out_exp = static_cast<int>(scale_byte) + exponent - 7;
    out_mantissa = mantissa << 4;
  }

  if (out_exp > 0 && out_exp < 255) {
    const uint16_t bf16_bits =
        sign | (static_cast<uint16_t>(out_exp) << 7) |
        static_cast<uint16_t>(out_mantissa);
    return __ushort_as_bfloat16(static_cast<unsigned short>(bf16_bits));
  }

  const float scaled =
      fp8_e4m3fn_to_float(bits) * exp2f(static_cast<float>(scale_byte) - 127.0f);
  return __float2bfloat16(scaled);
}

__device__ __forceinline__ __nv_bfloat16 approx_scaled_fp8_e4m3fn_to_bfloat16(
    uint8_t bits,
    uint8_t scale_byte) {
  const uint16_t sign = static_cast<uint16_t>(bits & 0x80) << 8;
  const uint16_t magnitude = static_cast<uint16_t>(bits & 0x7f);
  const int raw_exp = (bits >> 3) & 0x0f;
  const int exponent = raw_exp == 0 ? 1 : raw_exp;
  const int out_exp = static_cast<int>(scale_byte) + exponent - 7;
  const uint16_t bf16_bits =
      sign |
      (static_cast<uint16_t>(out_exp) << 7) |
      (static_cast<uint16_t>(bits & 0x07) << 4);
  const uint16_t max_finite = sign | 0x7f7f;
  const uint16_t normal_mask =
      static_cast<uint16_t>(0u - static_cast<unsigned>(magnitude != 0 && out_exp > 0 && out_exp < 255));
  const uint16_t overflow_mask =
      static_cast<uint16_t>(0u - static_cast<unsigned>(magnitude != 0 && out_exp >= 255));
  const uint16_t zero_mask = static_cast<uint16_t>(~(normal_mask | overflow_mask));
  return __ushort_as_bfloat16(static_cast<unsigned short>(
      (bf16_bits & normal_mask) | (max_finite & overflow_mask) | (sign & zero_mask)));
}
...
__device__ __forceinline__ int64_t prepared_p_tile_offset(
    int batch,
    int head_tiles,
    int row_tiles,
    int head_tile,
...
__global__ void ds4_cuda_prepare_k_tiles_kernel(
    ...
    int total_width,
    int row_tiles,
    __nv_bfloat16* __restrict__ prepared_k) {
  ...
  const int64_t tile_base =
      (((static_cast<int64_t>(batch) * row_tiles + row_tile) * kScoreKTileCount +
        dim_tile) *
       kPreparedKTileElems);
  for (int idx = threadIdx.x; idx < kPreparedKTileElems; idx += blockDim.x) {
    const int dim_slot = idx / kScoreRowsPerBlock;
    const int row_slot = idx - dim_slot * kScoreRowsPerBlock;
    const int dim = dim_base + dim_slot;
    __nv_bfloat16 value = __float2bfloat16(0.0f);
    if (row_valid[row_slot]) {
      const uint8_t* packed = row_ptrs[row_slot];
      if (dim < kNopeDim) {
        const int scale_slot = dim / kScaleGroup;
        const float decoded =
            fp8_e4m3fn_to_float(packed[dim]) * row_scales[row_slot][scale_slot];
        value = __float2bfloat16(decoded);
      } else {
        value = bf16_bytes_to_bfloat16(packed + kNopeDim + (dim - kNopeDim) * 2);
      }
    }
    prepared_k[tile_base + idx] = value;
  }
}

template <bool RowGroupPreparedK = false>
__global__ void ds4_cuda_prepare_kv_tiles_kernel(
    const uint8_t* __restrict__ swa_cache,
    const int32_t* __restrict__ swa_indices,
    const int32_t* __restrict__ swa_lengths,
    int swa_width,
    ...
    int row_tiles,
    __nv_bfloat16* __restrict__ prepared_k,
    __nv_bfloat16* __restrict__ prepared_v) {
  const int batch = blockIdx.x;
  const int row_tile = blockIdx.y;
  const int dim_tile = blockIdx.z;
  if (batch >= batch_size) {
  ...
  for (int idx = threadIdx.x; idx < kPreparedKTileElems; idx += blockDim.x) {
    ...
    if constexpr (RowGroupPreparedK) {
      const int row_group = row_slot / kScoreTileN;
      const int row_lane = row_slot - row_group * kScoreTileN;
      prepared_k[tile_base + row_group * kScoreTileElems +
                 dim_slot * kScoreTileN + row_lane] = value;
    } else {
      prepared_k[tile_base + idx] = value;
    }
    prepared_v[tile_base + row_slot * kScoreTileK + dim_slot] = value;
  }
}

__global__ void ds4_cuda_prepare_kv_tiles_common_cr4_kernel(
    const uint8_t* __restrict__ swa_cache,
    const int32_t* __restrict__ swa_indices,
    const int32_t* __restrict__ swa_lengths,
    const uint8_t* __restrict__ extra_cache,
  ...
  if (threadIdx.x < kScoreRowsPerBlock) {
    const int row_slot = threadIdx.x;
    const bool from_swa = row_tile < (kV56CommonSwaWidth / kScoreRowsPerBlock);
    const int local_row =
        from_swa ? row_tile * kScoreRowsPerBlock + row_slot
                 : (row_tile - (kV56CommonSwaWidth / kScoreRowsPerBlock)) *
                           kScoreRowsPerBlock +
                       row_slot;
    const int width =
        from_swa ? kV56CommonSwaWidth : kV56CommonExtraWidth;
    const int valid_len = clamped_length(
        from_swa ? swa_lengths : extra_lengths,
        batch,
        width);
    const bool valid = local_row < valid_len;
    const uint8_t* packed = nullptr;
    if (valid) {
      const int32_t raw_index =
          from_swa ? swa_indices[batch * kV56CommonSwaWidth + local_row]
                   : extra_indices[batch * kV56CommonExtraWidth + local_row];
      const int index = raw_index < 0 ? 0 : raw_index;
      if (from_swa) {
        const int page = index >> 8;
        const int token = index & (kV56CommonSwaPageSize - 1);
        packed = swa_cache +
                 (static_cast<int64_t>(page) * kV56CommonSwaPageSize + token) *
                     kV56CommonRowStride;
      } else {
        const int page = index >> 6;
        const int token = index & (kV56CommonExtraPageSize - 1);
        packed = extra_cache +
                 (static_cast<int64_t>(page) * kV56CommonExtraPageSize + token) *
                     kV56CommonRowStride;
      }
    }
    row_ptrs[row_slot] = packed;
    row_valid[row_slot] = valid ? 1 : 0;
  }
  __syncthreads();
...
template <bool KColMajorB = false, bool VColMajorB = false>
__global__ void ds4_cuda_prepare_kv_tiles_common_cr4_single_scale_kernel(
    const uint8_t* __restrict__ swa_cache,
    const int32_t* __restrict__ swa_indices,
    const int32_t* __restrict__ swa_lengths,
    const uint8_t* __restrict__ extra_cache,
  ...
  __shared__ const uint8_t* row_ptrs[kScoreRowsPerBlock];
  __shared__ uint8_t row_valid[kScoreRowsPerBlock];
  __shared__ float row_scale[kScoreRowsPerBlock];
  ...
  for (int idx = threadIdx.x; idx < kPreparedKTileElems; idx += blockDim.x) {
    ...
    if (row_valid[row_slot]) {
      ...
      if (nope_tile) {
        const float decoded =
            fp8_e4m3fn_to_float(packed[dim]) * row_scale[row_slot];
        value = __float2bfloat16(decoded);
      } else {
        value = bf16_bytes_to_bfloat16(packed + kNopeDim + (dim - kNopeDim) * 2);
      }
    ...
    const int row_lane = row_slot - row_group * kScoreTileN;
    const int k_idx =
        KColMajorB ? row_group * kScoreTileElems + row_lane * kScoreTileK +
                         dim_slot
                   : row_group * kScoreTileElems + dim_slot * kScoreTileN +
                         row_lane;
    const int v_idx =
        VColMajorB ? row_group * kScoreTileElems + dim_slot * kScoreTileN +
                         row_lane
                   : row_slot * kScoreTileK + dim_slot;
    prepared_k[tile_base + k_idx] = value;
    prepared_v[tile_base + v_idx] = value;
  }
}

__global__ void ds4_cuda_prepare_kv_tiles_row32_kernel(
    const uint8_t* __restrict__ swa_cache,
    const int32_t* __restrict__ swa_indices,
    const int32_t* __restrict__ swa_lengths,
    int swa_width,
  ...
  const int tile_row_base = row_tile * kV44ScoreRowsPerBlock;
  const int dim_base = dim_tile * kScoreTileK;
  __shared__ const uint8_t* row_ptrs[kV44ScoreRowsPerBlock];
  __shared__ uint8_t row_valid[kV44ScoreRowsPerBlock];
  __shared__ float row_scales[kV44ScoreRowsPerBlock][kScaleCount];

  if (threadIdx.x < kV44ScoreRowsPerBlock) {
    const int row = tile_row_base + threadIdx.x;
    bool valid = false;
    const uint8_t* packed = nullptr;
    if (row < total_width) {
  ...
  __syncthreads();
  for (int idx = threadIdx.x; idx < kV44ScoreRowsPerBlock * kScaleCount;
       idx += blockDim.x) {
    const int row_slot = idx / kScaleCount;
    const int scale_slot = idx - row_slot * kScaleCount;
    const uint8_t* packed = row_ptrs[row_slot];
  ...
  const int64_t tile_base =
      (((static_cast<int64_t>(batch) * row_tiles + row_tile) * kScoreKTileCount +
        dim_tile) *
       kV44PreparedKTileElems);
  for (int idx = threadIdx.x; idx < kV44PreparedKTileElems; idx += blockDim.x) {
    const int dim_slot = idx / kV44ScoreRowsPerBlock;
    const int row_slot = idx - dim_slot * kV44ScoreRowsPerBlock;
    const int dim = dim_base + dim_slot;
    __nv_bfloat16 value = __float2bfloat16(0.0f);
    if (row_valid[row_slot]) {
      const uint8_t* packed = row_ptrs[row_slot];
...
__global__ void ds4_cuda_fused_v36_dim_chunk_streamed_attention_kernel(
    const __nv_bfloat16* __restrict__ q,
    const uint8_t* __restrict__ swa_cache,
    const int32_t* __restrict__ swa_indices,
    const int32_t* __restrict__ swa_lengths,
  ...
  const int chunk_dim_base = dim_chunk * kV36DimChunk;
  const int warp = threadIdx.x >> 5;
  const int lane = threadIdx.x & 31;
  if (batch >= batch_size || dim_chunk >= kV36DimChunks) {
    return;
  }
  ...
  __shared__ __align__(16) float score_shared[kV11WarpsPerBlock][kScoreTileElems];
  __shared__ __align__(16) float weight_shared[kScoreWarpsPerBlock][kScoreTileElems];
  __shared__ __align__(16) float acc_shared[kV36AccElems];
  __shared__ float row_scales[kScoreRowsPerBlock][kScaleCount];
  __shared__ const uint8_t* row_ptrs[kScoreRowsPerBlock];
  __shared__ uint8_t row_valid[kScoreRowsPerBlock];
  __shared__ float running_max[kScoreTileM];
  ...
  for (int idx = threadIdx.x; idx < kV36AccElems; idx += blockDim.x) {
    acc_shared[idx] = 0.0f;
  }
  if (threadIdx.x < kScoreTileM) {
    running_max[threadIdx.x] = -INFINITY;
...
#pragma unroll
    ...
    for (int idx = threadIdx.x; idx < kV36AccElems; idx += blockDim.x) {
      const int head_slot = idx / kV36DimChunk;
      acc_shared[idx] *= old_scale_shared[head_slot];
    }
    __syncthreads();
    ...
    if (warp < kV36DimTileCount) {
      const int dim_base = chunk_dim_base + warp * kScoreTileN;
      const bool nope_tile = dim_base < kNopeDim;
      const int scale_slot = dim_base / kScaleGroup;
...
#pragma unroll
    ...
    for (int idx = threadIdx.x; idx < kV36DimTileCount * kScoreTileElems;
         idx += blockDim.x) {
      const int dim_tile = idx / kScoreTileElems;
      const int tile_idx = idx - dim_tile * kScoreTileElems;
      const int head_slot = tile_idx / kScoreTileN;
      ...
      const int head = head_base + head_slot;
      if (head < num_heads) {
        const int dim = dim_tile * kScoreTileN + dim_slot;
        acc_shared[head_slot * kV36DimChunk + dim] +=
            score_shared[dim_tile][tile_idx];
      }
    }
    __syncthreads();
  }
  ...
  for (int idx = threadIdx.x; idx < kV36AccElems; idx += blockDim.x) {
    const int head_slot = idx / kV36DimChunk;
    const int dim_in_chunk = idx - head_slot * kV36DimChunk;
    const int dim = chunk_dim_base + dim_in_chunk;
    const int head = head_base + head_slot;
    if (head < num_heads) {
      const int64_t out_offset =
          (static_cast<int64_t>(batch) * num_heads + head) * kHeadDim + dim;
...
__global__ void ds4_cuda_fused_v37_shared_qk_direct_acc_attention_kernel(
    const __nv_bfloat16* __restrict__ q,
    const uint8_t* __restrict__ swa_cache,
    const int32_t* __restrict__ swa_indices,
    const int32_t* __restrict__ swa_lengths,
  ...
  __shared__ __align__(16) __nv_bfloat16
      p_shared[kScoreWarpsPerBlock][kScoreTileElems];
  __shared__ __align__(16) __nv_bfloat16
      k_shared[kV37DimTilesPerRound][kScoreTileElems];
  __shared__ __align__(16) float score_shared[kScoreWarpsPerBlock][kScoreTileElems];
  __shared__ __align__(16) float weight_shared[kScoreWarpsPerBlock][kScoreTileElems];
  __shared__ __align__(16) float acc_shared[kV37AccElems];
  __shared__ float row_scales[kScoreRowsPerBlock][kScaleCount];
  __shared__ const uint8_t* row_ptrs[kScoreRowsPerBlock];
  __shared__ uint8_t row_valid[kScoreRowsPerBlock];
  __shared__ float running_max[kScoreTileM];
  ...
  for (int idx = threadIdx.x; idx < kV37AccElems; idx += blockDim.x) {
    acc_shared[idx] = 0.0f;
  }
  if (threadIdx.x < kScoreTileM) {
    running_max[threadIdx.x] = -INFINITY;
...
#pragma unroll
    ...
    for (int dim_tile_base = 0; dim_tile_base < kV37DimTileCount;
         dim_tile_base += kV37DimTilesPerRound) {
      const int dim_tile = dim_tile_base + warp;
      if (warp < kV37DimTilesPerRound && dim_tile < kV37DimTileCount) {
        const int dim_base = dim_tile * kScoreTileN;
        const bool nope_tile = dim_base < kNopeDim;
        const int scale_slot = dim_base / kScaleGroup;
...
__global__ void ds4_cuda_fused_v8_mma_partial_kernel(
    ...
    const __nv_bfloat16* __restrict__ prepared_k = nullptr,
    const __nv_bfloat16* __restrict__ prepared_v = nullptr) {
  namespace wmma = nvcuda::wmma;
  ...
  constexpr int kPartialRowsPerBlock = kScoreTileN * kPartialScoreWarps;
  constexpr int kPartialPreparedKTileElems =
      kV44Row32 ? kV44PreparedKTileElems : kPreparedKTileElems;
  constexpr int kPartialPreparedVTileElems =
      kV44Row32 ? kV44PreparedVTileElems : kPreparedVTileElems;
  constexpr int kPartialPvDimRoundGroups =
      kHalfPvWarpMode ? kV47DimRoundGroups : kV15DimRoundGroups;
  constexpr int kPartialPvOuterRounds =
      kHalfPvWarpMode ? kV47DimOuterRounds : kV15DimOuterRounds;
  constexpr int kPartialPvWarps =
      kHalfPvWarpMode ? kV47WarpsPerBlock : kV11WarpsPerBlock;
  const int tile_row_base = row_tile * kPartialRowsPerBlock;
  const int warp = threadIdx.x >> 5;
  const int lane = threadIdx.x & 31;
  if (batch >= batch_size) {
    return;
  ...
  constexpr int kPartialSharedWarps =
      PVMmaMode >= 2 ? kPartialPvWarps : kScoreWarpsPerBlock;
  __shared__ __align__(16) __nv_bfloat16
      q_shared[kPartialSharedWarps][kScoreTileM * kScoreTileK];
  __shared__ __align__(16) __nv_bfloat16
      q_reuse_shared[kRollingQReuseMode ? 1 : (kFullQReuseMode ? kScoreKTileCount : 1)]
                    [kScoreTileElems];
  __shared__ __align__(16) __nv_bfloat16
  ...
  __shared__ __align__(16) float
      weight_shared[kDirectPCache ? 1 : kScoreWarpsPerBlock][kScoreTileElems];
  __shared__ __align__(16) __nv_bfloat16
      p_shared[kDirectPCache ? kPartialScoreWarps : 1][kScoreTileElems];
  __shared__ float row_scales[CacheScales ? kPartialRowsPerBlock : 1][kScaleCount];
  __shared__ uint8_t row_scale_bytes[(PVMmaMode == 12 || PVMmaMode == 13)
                                         ? kPartialRowsPerBlock
                                         : 1]
                                    [kScaleCount];
  __shared__ const uint8_t* row_ptrs[kPartialRowsPerBlock];
  __shared__ uint8_t row_valid[kPartialRowsPerBlock];
  ...
  if (threadIdx.x < kPartialRowsPerBlock) {
    const int row = tile_row_base + threadIdx.x;
    bool valid = false;
    const uint8_t* packed = nullptr;
    if (row < total_width) {
  ...
  if constexpr (CacheScales) {
    if (threadIdx.x < kPartialRowsPerBlock * kScaleCount) {
      const int row_slot = threadIdx.x / kScaleCount;
      const int scale_slot = threadIdx.x - row_slot * kScaleCount;
      const uint8_t* packed = row_ptrs[row_slot];
      if constexpr (PVMmaMode == 12 || PVMmaMode == 13) {
        row_scale_bytes[row_slot][scale_slot] =
            row_valid[row_slot] ? packed[kScaleOffset + scale_slot] : 127;
      } else {
        row_scales[row_slot][scale_slot] =
            row_valid[row_slot]
                ? exp2f(static_cast<float>(packed[kScaleOffset + scale_slot]) - 127.0f)
  ...
  if (warp < kPartialScoreWarps || kRollingQReuseMode) {
    wmma::fragment<wmma::matrix_a, kScoreTileM, kScoreTileN, kScoreTileK, __nv_bfloat16, wmma::row_major>
        q_frag;
    wmma::fragment<wmma::matrix_b, kScoreTileM, kScoreTileN, kScoreTileK, __nv_bfloat16, wmma::row_major>
        k_frag;
    ...
    for (int dim_base = 0; dim_base < kHeadDim; dim_base += kScoreTileK) {
      ...
      if constexpr (kRollingQReuseMode) {
      ...
      if (warp < kPartialScoreWarps) {
      if constexpr (PVMmaMode == 9 || PVMmaMode == 10 || PVMmaMode == 11 ||
                    PVMmaMode == 12 || PVMmaMode == 13 || PVMmaMode == 14 ||
...
#pragma unroll
      ...
      if constexpr (kQReuseMode) {
        wmma::load_matrix_sync(
            q_frag,
            q_reuse_shared[kRollingQReuseMode ? 0 : (dim_base / kScoreTileK)],
            kScoreTileK);
      } else {
        wmma::load_matrix_sync(q_frag, q_shared[warp], kScoreTileK);
      }
  ...
  if (threadIdx.x < kScoreTileM) {
    ...
    float tile_max = -INFINITY;
#pragma unroll
    for (int row_slot = 0; row_slot < kPartialRowsPerBlock; ++row_slot) {
      if (head < num_heads && row_valid[row_slot]) {
        const int row_warp = row_slot / kScoreTileN;
        const int row_lane = row_slot - row_warp * kScoreTileN;
        const float score =
...
#pragma unroll
  ...
  if constexpr (PVMmaMode == 5 || PVMmaMode == 6 || PVMmaMode == 7 ||
    ...
    // warps; v47 halves that to 8 warps and doubles the outer dim rounds.
    wmma::fragment<wmma::matrix_a, kScoreTileM, kScoreTileN, kScoreTileK, __nv_bfloat16, wmma::row_major>
        p_frag;
    wmma::fragment<wmma::matrix_b, kScoreTileM, kScoreTileN, kScoreTileK, __nv_bfloat16, wmma::row_major>
        v_frag;
    ...
    for (int outer_round = 0; outer_round < kPartialPvOuterRounds; ++outer_round) {
      if (warp < kPartialPvWarps) {
        const int dim_round_group = warp / kV11DimGroups;
        const int dim_group = warp - dim_round_group * kV11DimGroups;
        const int dim_round = outer_round * kPartialPvDimRoundGroups + dim_round_group;
        const int dim_base = (dim_round * kV11DimGroups + dim_group) * kScoreTileN;
        const bool nope_tile = dim_base < kNopeDim;
        const int scale_slot = dim_base / kScaleGroup;

        float pv_acc_inline[8];
        if constexpr (PVMmaMode == 11) {
#pragma unroll
          for (int i = 0; i < 8; ++i) {
            pv_acc_inline[i] = 0.0f;
          }
        } else {
          wmma::fill_fragment(pv_acc_frag, 0.0f);
        }
#pragma unroll
        for (int row_group = 0; row_group < kPartialScoreWarps; ++row_group) {
          ...
          if constexpr (PVMmaMode == 11) {
            ...
            uint32_t v_regs[4];
            ldmatrix_m8n8x4_b16(
                ldmatrix_16x16_rowmajor_ptr(q_shared[row_group], lane), p_regs);
            ldmatrix_m8n8x4_trans_b16(
                ldmatrix_16x16_rowmajor_ptr(k_shared[warp], lane), v_regs);
            mma_m16n8k16_bf16(pv_acc_inline, p_regs, v_regs[0], v_regs[1]);
            mma_m16n8k16_bf16(pv_acc_inline + 4, p_regs, v_regs[2], v_regs[3]);
          } else {
            if constexpr (kDirectPCache) {
              wmma::load_matrix_sync(p_frag, p_shared[row_group], kScoreTileN);
            } else {
...
#pragma unroll
            for (int reg = 0; reg < 4; ++reg) {
              const int head_slot = row_base + ((reg >= 2) ? 8 : 0);
              const int dim_slot = n_half * 8 + col_pair + (reg & 1);
              const int head = head_base + head_slot;
              const int dim = dim_base + dim_slot;
              const int64_t acc_offset =
                  ((((static_cast<int64_t>(batch) * head_tiles + head_tile) * row_tiles + row_tile) *
              ...
              store_partial_acc(
                  partial_acc,
                  acc_offset,
                  head < num_heads ? pv_acc_inline[n_half * 4 + reg] : 0.0f);
            }
          }
        } else {
          wmma::store_matrix_sync(
...
template <bool ProfileStages = false>
__global__ void __launch_bounds__(kScoreThreads, 1)
ds4_cuda_fused_v48_prepared_p_grouped_qk_kernel(
    const __nv_bfloat16* __restrict__ q,
    const uint8_t* __restrict__ swa_cache,
    const int32_t* __restrict__ swa_indices,
    const int32_t* __restrict__ swa_lengths,
    ...
    float* __restrict__ partial_max,
    float* __restrict__ partial_sum,
    __nv_bfloat16* __restrict__ prepared_p,
    unsigned long long* __restrict__ profile_cycles) {
  namespace wmma = nvcuda::wmma;

  const int batch = blockIdx.x;
  const int head_group = blockIdx.y;
  const int first_head_tile = head_group * kV48HeadTilesPerBlock;
  const int row_tile = blockIdx.z;
  const int tile_row_base = row_tile * kScoreRowsPerBlock;
  const int warp = threadIdx.x >> 5;
  const int lane = threadIdx.x & 31;
  ...
  __shared__ __align__(16) __nv_bfloat16
      q_reuse_shared[kV48HeadTilesPerBlock][kScoreKTileCount][kScoreTileElems];
  __shared__ __align__(16) __nv_bfloat16 k_shared[kScoreWarpsPerBlock][kScoreTileElems];
  __shared__ __align__(16) float
      qk_score_shared[kV48HeadTilesPerBlock][kScoreWarpsPerBlock][kScoreTileElems];
  __shared__ uint8_t row_valid[kScoreRowsPerBlock];
  ...
  for (int idx = threadIdx.x;
       idx < kV48HeadTilesPerBlock * kScoreKTileCount * kScoreTileElems;
       idx += blockDim.x) {
    const int group = idx / (kScoreKTileCount * kScoreTileElems);
    const int group_idx = idx - group * kScoreKTileCount * kScoreTileElems;
    const int dim_tile = group_idx / kScoreTileElems;
  ...
  if (warp < kScoreWarpsPerBlock) {
    ...
    const int warp_row_base = warp * kScoreTileN;
    unsigned long long qk_thread_k_load_cycles = 0;
    for (int dim_tile = 0; dim_tile < kScoreKTileCount; ++dim_tile) {
      unsigned long long qk_detail_t0 = 0;
      if (ProfileStages && threadIdx.x == 0) {
        qk_detail_t0 = clock64();
      }
      ...
      for (int idx = lane; idx < kScoreTileElems; idx += 32) {
        ...
        const int local_row = warp_row_base + k_row_slot;
        k_shared[warp][idx] =
            prepared_k[prepared_base + k_dim_slot * kScoreRowsPerBlock + local_row];
      }
      if (ProfileStages && threadIdx.x == 0) {
        const unsigned long long now = clock64();
        qk_thread_k_load_cycles += now - qk_detail_t0;
      }
      __syncwarp();
      if (ProfileStages && threadIdx.x == 0) {
        const unsigned long long now = clock64();
      ...
      wmma::load_matrix_sync(k_frag, k_shared[warp], kScoreTileN);
      wmma::load_matrix_sync(q_frag, q_reuse_shared[0][dim_tile], kScoreTileK);
      wmma::mma_sync(acc_frag0, q_frag, k_frag, acc_frag0);
      wmma::load_matrix_sync(q_frag, q_reuse_shared[1][dim_tile], kScoreTileK);
      wmma::mma_sync(acc_frag1, q_frag, k_frag, acc_frag1);
      __syncwarp();
      if (ProfileStages && threadIdx.x == 0) {
        const unsigned long long now = clock64();
    ...
    if (ProfileStages && threadIdx.x == 0) {
      ...
      add_partial_profile_cycles(
          profile_cycles, kPartialProfileQkScoreStoreWall, now - qk_store_t0);
      add_partial_profile_cycles(
          profile_cycles, kPartialProfileQkThreadKDecode, qk_thread_k_load_cycles);
      qk_barrier_t0 = now;
    }
  }
  __syncthreads();
  ...
  for (int idx = threadIdx.x; idx < kV48HeadTilesPerBlock * kScoreTileM;
       idx += blockDim.x) {
    const int group = idx / kScoreTileM;
    const int head_slot = idx - group * kScoreTileM;
    const int head_tile = first_head_tile + group;
...
#pragma unroll
  ...
  for (int idx = threadIdx.x;
       idx < kV48HeadTilesPerBlock * kScoreWarpsPerBlock * kScoreTileElems;
       idx += blockDim.x) {
    const int group = idx / (kScoreWarpsPerBlock * kScoreTileElems);
    const int rem = idx - group * kScoreWarpsPerBlock * kScoreTileElems;
    const int row_group = rem / kScoreTileElems;
    const int tile_idx = rem - row_group * kScoreTileElems;
    const int head_tile = first_head_tile + group;
    if (head_tile < head_tiles) {
      prepared_p[prepared_p_tile_offset(
          batch, head_tiles, row_tiles, head_tile, row_tile, row_group, tile_idx)] =
          __float2bfloat16(qk_score_shared[group][row_group][tile_idx]);
    }
  }
  __syncthreads();
...
template <typename PartialAccT = __nv_bfloat16, bool ProfileStages = false>
__global__ void __launch_bounds__(kV48Threads, 1)
ds4_cuda_fused_v48_prepared_p_consumer_partial_kernel(
    const __nv_bfloat16* __restrict__ prepared_p,
  ...
  if (warp < kScoreWarpsPerBlock) {
    for (int idx = lane; idx < kScoreTileElems; idx += 32) {
      p_shared[warp][idx] = prepared_p[prepared_p_tile_offset(
          batch, head_tiles, row_tiles, head_tile, row_tile, warp, idx)];
    }
  }
  __syncthreads();
  if (ProfileStages && threadIdx.x == 0) {
  ...
  for (int outer_round = 0; outer_round < kV15DimOuterRounds; ++outer_round) {
    if (warp < kV11WarpsPerBlock) {
      ...
      const int dim_group = warp - dim_round_group * kV11DimGroups;
      const int dim_round = outer_round * kV15DimRoundGroups + dim_round_group;
      const int dim_tile = dim_round * kV11DimGroups + dim_group;
      const int64_t prepared_base =
          (((static_cast<int64_t>(batch) * row_tiles + row_tile) * kScoreKTileCount +
            dim_tile) *
           kPreparedVTileElems);
...
#pragma unroll
      for (int row_group = 0; row_group < kScoreWarpsPerBlock; ++row_group) {
        for (int idx = lane; idx < kScoreTileElems; idx += 32) {
          ...
          const int row_slot = row_group * kScoreTileN + k_slot;
          v_shared[warp][idx] =
              prepared_v[prepared_base + row_slot * kScoreTileK + dim_slot];
        }
        __syncwarp();
...
torch::Tensor launch_ds4_cuda_attention(
  ...
  dim3 v36_grid(
      static_cast<unsigned int>(batch_size),
      static_cast<unsigned int>((num_heads + kScoreTileM - 1) / kScoreTileM),
      static_cast<unsigned int>(kV36DimChunks));
  dim3 block(kThreads);
  dim3 score_block(kScoreThreads);
  dim3 fused_block(kV7Threads);
  dim3 v36_block(kV36Threads);
  dim3 v37_block(kV37Threads);
  ...
  if (variant == AttentionVariant::kOptimizedV55 ||
      variant == AttentionVariant::kOptimizedV56 ||
      variant == AttentionVariant::kOptimizedV57 ||
      variant == AttentionVariant::kOptimizedV58A ||
    ...
    const bool common_cr4_path =
        has_extra &&
        num_heads == 64 &&
        swa_width == kV56CommonSwaWidth &&
        extra_width == kV56CommonExtraWidth &&
        total_width == kV56CommonSwaWidth + kV56CommonExtraWidth &&
        swa_page_size == kV56CommonSwaPageSize &&
        extra_page_size == kV56CommonExtraPageSize &&
        swa_k_cache.size(3) == kV56CommonRowStride &&
        extra_k_cache.size(3) == kV56CommonRowStride;
    if (!common_cr4_path) {
      variant = AttentionVariant::kOptimizedV53;
    }
  }
  switch (variant) {
    case AttentionVariant::kReference:
    ...
    case AttentionVariant::kOptimizedV25:
      ...
      break;
    case AttentionVariant::kOptimizedV36:
      ds4_cuda_fused_v36_dim_chunk_streamed_attention_kernel
          <<<v36_grid, v36_block, 0, at::cuda::getCurrentCUDAStream()>>>(
              reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
              swa_k_cache.data_ptr<uint8_t>(),
              swa_indices.data_ptr<int32_t>(),
              swa_topk_lengths.data_ptr<int32_t>(),
      ...
      break;
    case AttentionVariant::kOptimizedV37:
      ds4_cuda_fused_v37_shared_qk_direct_acc_attention_kernel
          <<<fused_grid, v37_block, 0, at::cuda::getCurrentCUDAStream()>>>(
              reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
              swa_k_cache.data_ptr<uint8_t>(),
              swa_indices.data_ptr<int32_t>(),
              swa_topk_lengths.data_ptr<int32_t>(),
    ...
    case AttentionVariant::kOptimizedV60: {
      ...
      const bool tensor_core_pv_v50_family =
          variant == AttentionVariant::kOptimizedV50 ||
          variant == AttentionVariant::kOptimizedV51 ||
          variant == AttentionVariant::kOptimizedV52 ||
          variant == AttentionVariant::kOptimizedV53 ||
          variant == AttentionVariant::kOptimizedV54 ||
          variant == AttentionVariant::kOptimizedV55 ||
          variant == AttentionVariant::kOptimizedV56 ||
          variant == AttentionVariant::kOptimizedV57 ||
          variant == AttentionVariant::kOptimizedV58A ||
          variant == AttentionVariant::kOptimizedV58B ||
      ...
      const bool tensor_core_pv_rowgroup_accum =
          ...
          variant == AttentionVariant::kOptimizedV38 ||
          variant == AttentionVariant::kOptimizedV39 ||
          variant == AttentionVariant::kOptimizedV40 ||
          variant == AttentionVariant::kOptimizedV41 ||
          variant == AttentionVariant::kOptimizedV42 ||
          variant == AttentionVariant::kOptimizedV43 ||
      ...
      const bool tensor_core_pv_lane_row_k_staging =
          variant == AttentionVariant::kOptimizedV27;
      const bool tensor_core_pv_direct_acc_store =
          variant == AttentionVariant::kOptimizedV38;
      const bool tensor_core_pv_inline_mma_store =
          variant == AttentionVariant::kOptimizedV39;
      const bool tensor_core_pv_direct_scaled_fp8 =
          variant == AttentionVariant::kOptimizedV40;
      const bool tensor_core_pv_approx_scaled_fp8 =
          variant == AttentionVariant::kOptimizedV41;
      const bool tensor_core_pv_prepared_k =
          variant == AttentionVariant::kOptimizedV42 ||
          variant == AttentionVariant::kOptimizedV43 ||
          variant == AttentionVariant::kOptimizedV44 ||
          variant == AttentionVariant::kOptimizedV45 ||
      ...
      const bool tensor_core_pv_prepared_kv_half_pv_warps =
          variant == AttentionVariant::kOptimizedV47;
      const bool tensor_core_pv_prepared_kv =
          variant == AttentionVariant::kOptimizedV43 ||
          variant == AttentionVariant::kOptimizedV44 ||
          variant == AttentionVariant::kOptimizedV45 ||
          variant == AttentionVariant::kOptimizedV46 ||
      ...
      const bool tensor_core_pv_prepared_kv_row32 =
          variant == AttentionVariant::kOptimizedV44;
      const bool tensor_core_pv_prepared_kv_rowgroup_k =
          variant == AttentionVariant::kOptimizedV52 ||
          variant == AttentionVariant::kOptimizedV53 ||
          variant == AttentionVariant::kOptimizedV54 ||
          variant == AttentionVariant::kOptimizedV55 ||
      ...
      const bool tensor_core_pv_v50_direct_prepared_wmma =
          variant == AttentionVariant::kOptimizedV50;
      const bool tensor_core_pv_v51_warp_softmax_direct_prepared_wmma =
          variant == AttentionVariant::kOptimizedV51;
      const bool tensor_core_pv_v52_rowgroup_k_direct_prepared_wmma =
          variant == AttentionVariant::kOptimizedV52;
      const bool tensor_core_pv_v53_warp_softmax_rowgroup_k_direct_prepared_wmma =
          variant == AttentionVariant::kOptimizedV53 ||
          variant == AttentionVariant::kOptimizedV56 ||
          variant == AttentionVariant::kOptimizedV57;
      const bool tensor_core_pv_v54_staged_k_warp_softmax_rowgroup_direct_prepared_wmma =
          variant == AttentionVariant::kOptimizedV54;
      const bool tensor_core_pv_v55_dimsplit_qk_warp_softmax_rowgroup_direct_prepared_wmma =
          variant == AttentionVariant::kOptimizedV55;
      const bool tensor_core_pv_v56_common_prepared_kv_producer =
          variant == AttentionVariant::kOptimizedV56;
      const bool tensor_core_pv_v57_single_scale_prepared_kv_producer =
          variant == AttentionVariant::kOptimizedV57;
      const bool tensor_core_pv_v58a_k_colmajor_b_direct_prepared_wmma =
          variant == AttentionVariant::kOptimizedV58A;
      const bool tensor_core_pv_v58b_v_colmajor_b_direct_prepared_wmma =
          variant == AttentionVariant::kOptimizedV58B;
      const bool tensor_core_pv_v58_kv_colmajor_b_direct_prepared_wmma =
          variant == AttentionVariant::kOptimizedV58;
      const bool tensor_core_pv_v59_a_colmajor_direct_prepared_wmma =
          variant == AttentionVariant::kOptimizedV59;
      ...
      const int64_t rows_per_partial_tile =
          (tensor_core_pv_row32_independent_grouped_head_k_reuse ||
           tensor_core_pv_prepared_kv_row32)
      ...
      torch::Tensor prepared_k;
      const int64_t prepared_k_tile_elems =
          tensor_core_pv_prepared_kv_row32 ? kV44PreparedKTileElems
                                           : kPreparedKTileElems;
      const int64_t prepared_v_tile_elems =
          tensor_core_pv_prepared_kv_row32 ? kV44PreparedVTileElems
                                           : kPreparedVTileElems;
      if (tensor_core_pv_prepared_k) {
        TORCH_CHECK(
            row_tiles > 0,
            "prepared-K attention requires at least one selected row tile");
        prepared_k = torch::empty(
            {batch_size,
             row_tiles,
             static_cast<int64_t>(kScoreKTileCount),
             prepared_k_tile_elems},
            q.options().dtype(torch::kBFloat16));
      }
      torch::Tensor prepared_v;
      if (tensor_core_pv_prepared_kv) {
        prepared_v = torch::empty(
            {batch_size,
             row_tiles,
             static_cast<int64_t>(kScoreKTileCount),
             prepared_v_tile_elems},
            q.options().dtype(torch::kBFloat16));
      }
      torch::Tensor prepared_p;
      if (tensor_core_pv_prepared_p_split) {
        prepared_p = torch::empty(
            {batch_size,
             head_tiles,
             row_tiles,
             static_cast<int64_t>(kScoreWarpsPerBlock),
             static_cast<int64_t>(kScoreTileElems)},
            q.options().dtype(torch::kBFloat16));
      }
      ...
      const int64_t partial_head_blocks =
          (tensor_core_pv_grouped_head_k_reuse || tensor_core_pv_score_split ||
           ...
           tensor_core_pv_prepared_p_split)
      ...
      dim3 v47_block(kV47Threads);
      const auto stream = at::cuda::getCurrentCUDAStream();
      const bool profile_split = split_profile_enabled();
      const bool tensor_core_pv_has_producer =
          tensor_core_pv_score_split || tensor_core_pv_prepared_k;
      ...
      if (profile_split) {
        ...
        if (tensor_core_pv_has_producer) {
      ...
      if (tensor_core_pv_prepared_k) {
        const dim3 prepare_grid(
            static_cast<unsigned int>(batch_size),
            static_cast<unsigned int>(row_tiles),
            static_cast<unsigned int>(kScoreKTileCount));
        const dim3 prepare_block(kScoreThreads);
        if (tensor_core_pv_prepared_kv_row32) {
          ds4_cuda_prepare_kv_tiles_row32_kernel<<<prepare_grid, prepare_block, 0, stream>>>(
              swa_k_cache.data_ptr<uint8_t>(),
              swa_indices.data_ptr<int32_t>(),
              swa_topk_lengths.data_ptr<int32_t>(),
              static_cast<int>(swa_width),
              ...
              reinterpret_cast<__nv_bfloat16*>(prepared_k.data_ptr<at::BFloat16>()),
              reinterpret_cast<__nv_bfloat16*>(prepared_v.data_ptr<at::BFloat16>()));
        } else if (tensor_core_pv_v58_layout_prepared_kv_producer) {
#define DSV4_LAUNCH_COMMON_SINGLE_SCALE_LAYOUT(K_COL_B, V_COL_B)                  \
  ds4_cuda_prepare_kv_tiles_common_cr4_single_scale_kernel<K_COL_B, V_COL_B>      \
      <<<prepare_grid, prepare_block, 0, stream>>>(                               \
          swa_k_cache.data_ptr<uint8_t>(),                                        \
          swa_indices.data_ptr<int32_t>(),                                        \
          swa_topk_lengths.data_ptr<int32_t>(),                                   \
          extra_k_cache.data_ptr<uint8_t>(),                                      \
          extra_indices.data_ptr<int32_t>(),                                      \
          extra_topk_lengths.data_ptr<int32_t>(),                                 \
          static_cast<int>(batch_size),                                           \
          static_cast<int>(row_tiles),                                            \
          reinterpret_cast<__nv_bfloat16*>(prepared_k.data_ptr<at::BFloat16>()),  \
          reinterpret_cast<__nv_bfloat16*>(prepared_v.data_ptr<at::BFloat16>()))
...
#undef DSV4_LAUNCH_COMMON_SINGLE_SCALE_LAYOUT
        } else if (
            tensor_core_pv_v57_single_scale_prepared_kv_producer ||
            tensor_core_pv_v59_a_colmajor_direct_prepared_wmma) {
          ds4_cuda_prepare_kv_tiles_common_cr4_single_scale_kernel<false, false><<<prepare_grid, prepare_block, 0, stream>>>(
              swa_k_cache.data_ptr<uint8_t>(),
              swa_indices.data_ptr<int32_t>(),
              swa_topk_lengths.data_ptr<int32_t>(),
              extra_k_cache.data_ptr<uint8_t>(),
              ...
              reinterpret_cast<__nv_bfloat16*>(prepared_k.data_ptr<at::BFloat16>()),
              reinterpret_cast<__nv_bfloat16*>(prepared_v.data_ptr<at::BFloat16>()));
        } else if (tensor_core_pv_v56_common_prepared_kv_producer) {
          ds4_cuda_prepare_kv_tiles_common_cr4_kernel<<<prepare_grid, prepare_block, 0, stream>>>(
              swa_k_cache.data_ptr<uint8_t>(),
              swa_indices.data_ptr<int32_t>(),
              swa_topk_lengths.data_ptr<int32_t>(),
              extra_k_cache.data_ptr<uint8_t>(),
              ...
              reinterpret_cast<__nv_bfloat16*>(prepared_k.data_ptr<at::BFloat16>()),
              reinterpret_cast<__nv_bfloat16*>(prepared_v.data_ptr<at::BFloat16>()));
        } else if (tensor_core_pv_prepared_kv) {
#define DSV4_LAUNCH_PREPARE_KV(ROWGROUP_K)                                      \
  ds4_cuda_prepare_kv_tiles_kernel<ROWGROUP_K>                                   \
      <<<prepare_grid, prepare_block, 0, stream>>>(                              \
          swa_k_cache.data_ptr<uint8_t>(),                                       \
          swa_indices.data_ptr<int32_t>(),                                       \
          swa_topk_lengths.data_ptr<int32_t>(),                                  \
          ...
          reinterpret_cast<__nv_bfloat16*>(prepared_k.data_ptr<at::BFloat16>()), \
          reinterpret_cast<__nv_bfloat16*>(prepared_v.data_ptr<at::BFloat16>()))
          if (tensor_core_pv_prepared_kv_rowgroup_k) {
            DSV4_LAUNCH_PREPARE_KV(true);
          } else {
            DSV4_LAUNCH_PREPARE_KV(false);
          }
#undef DSV4_LAUNCH_PREPARE_KV
        } else {
          ds4_cuda_prepare_k_tiles_kernel<<<prepare_grid, prepare_block, 0, stream>>>(
              swa_k_cache.data_ptr<uint8_t>(),
              swa_indices.data_ptr<int32_t>(),
              swa_topk_lengths.data_ptr<int32_t>(),
              static_cast<int>(swa_width),
        ...
        if (profile_split && !tensor_core_pv_prepared_p_split) {
      ...
      if (tensor_core_pv_rowgroup_accum) {
        if (tensor_core_pv_independent_grouped_head_k_reuse) {
        ...
        } else if (tensor_core_pv_prepared_p_split) {
          const __nv_bfloat16* prepared_k_ptr =
              reinterpret_cast<const __nv_bfloat16*>(
                  prepared_k.data_ptr<at::BFloat16>());
          const __nv_bfloat16* prepared_v_ptr =
              reinterpret_cast<const __nv_bfloat16*>(
                  prepared_v.data_ptr<at::BFloat16>());
          __nv_bfloat16* prepared_p_ptr =
              reinterpret_cast<__nv_bfloat16*>(prepared_p.data_ptr<at::BFloat16>());
          if (profile_partial_stages) {
            ds4_cuda_fused_v48_prepared_p_grouped_qk_kernel<true>
            ...
            ds4_cuda_fused_v48_prepared_p_grouped_qk_kernel<false>
                <<<split_grid, score_block, 0, stream>>>(
                    reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                    swa_k_cache.data_ptr<uint8_t>(),
                    swa_indices.data_ptr<int32_t>(),
          ...
          }
          {
            ...
            TORCH_CHECK(
                err == cudaSuccess,
                "v48 prepared-P producer launch failed: ",
                cudaGetErrorString(err));
          }
          if (profile_partial_stages) {
            ds4_cuda_fused_v48_prepared_p_consumer_partial_kernel<
                __nv_bfloat16,
                true><<<consumer_grid, split_block, 0, stream>>>(
                prepared_p_ptr,
                prepared_v_ptr,
                static_cast<int>(batch_size),
                static_cast<int>(num_heads),
            ...
            ds4_cuda_fused_v48_prepared_p_consumer_partial_kernel<__nv_bfloat16>
          ...
          }
          {
            ...
            TORCH_CHECK(
                err == cudaSuccess,
                "v48 prepared-P consumer launch failed: ",
                cudaGetErrorString(err));
          }
        ...
        } else if (tensor_core_pv_direct_scaled_fp8) {
          if (profile_partial_stages) {
            ds4_cuda_fused_v8_mma_partial_kernel<true, 12, __nv_bfloat16, true>
                <<<split_grid, split_block, 0, stream>>>(
                    reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                    swa_k_cache.data_ptr<uint8_t>(),
                    swa_indices.data_ptr<int32_t>(),
            ...
            ds4_cuda_fused_v8_mma_partial_kernel<true, 12, __nv_bfloat16>
                <<<split_grid, split_block, 0, stream>>>(
                    reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                    swa_k_cache.data_ptr<uint8_t>(),
                    swa_indices.data_ptr<int32_t>(),
        ...
        } else if (tensor_core_pv_approx_scaled_fp8) {
          if (profile_partial_stages) {
            ds4_cuda_fused_v8_mma_partial_kernel<true, 13, __nv_bfloat16, true>
                <<<split_grid, split_block, 0, stream>>>(
                    reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                    swa_k_cache.data_ptr<uint8_t>(),
                    swa_indices.data_ptr<int32_t>(),
            ...
            ds4_cuda_fused_v8_mma_partial_kernel<true, 13, __nv_bfloat16>
                <<<split_grid, split_block, 0, stream>>>(
                    reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                    swa_k_cache.data_ptr<uint8_t>(),
                    swa_indices.data_ptr<int32_t>(),
        ...
        } else if (tensor_core_pv_prepared_k) {
          const __nv_bfloat16* prepared_k_ptr =
              reinterpret_cast<const __nv_bfloat16*>(
                  prepared_k.data_ptr<at::BFloat16>());
          if (tensor_core_pv_prepared_kv) {
            ...
            if (tensor_core_pv_prepared_kv_row32) {
              if (profile_partial_stages) {
                ds4_cuda_fused_v8_mma_partial_kernel<false, 16, __nv_bfloat16, true>
                    <<<split_grid, split_block, 0, stream>>>(
                        reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                        swa_k_cache.data_ptr<uint8_t>(),
                        swa_indices.data_ptr<int32_t>(),
                ...
                ds4_cuda_fused_v8_mma_partial_kernel<false, 16, __nv_bfloat16>
                    <<<split_grid, split_block, 0, stream>>>(
                        reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                        swa_k_cache.data_ptr<uint8_t>(),
                        swa_indices.data_ptr<int32_t>(),
                ...
                tensor_core_pv_v50_direct_prepared_wmma ||
                tensor_core_pv_v51_warp_softmax_direct_prepared_wmma ||
                tensor_core_pv_v52_rowgroup_k_direct_prepared_wmma ||
                tensor_core_pv_v53_warp_softmax_rowgroup_k_direct_prepared_wmma ||
                tensor_core_pv_v54_staged_k_warp_softmax_rowgroup_direct_prepared_wmma ||
                tensor_core_pv_v55_dimsplit_qk_warp_softmax_rowgroup_direct_prepared_wmma ||
                tensor_core_pv_v58a_k_colmajor_b_direct_prepared_wmma ||
                tensor_core_pv_v58b_v_colmajor_b_direct_prepared_wmma ||
                tensor_core_pv_v58_kv_colmajor_b_direct_prepared_wmma ||
                tensor_core_pv_v59_a_colmajor_direct_prepared_wmma ||
              ...
              if (tensor_core_pv_v51_warp_softmax_direct_prepared_wmma) {
                launch_standalone_direct_prepared =
                    ds4_cuda_launch_v51_warp_softmax_direct_prepared_wmma_partial;
              }
              if (tensor_core_pv_v53_warp_softmax_rowgroup_k_direct_prepared_wmma) {
                launch_standalone_direct_prepared =
                    ds4_cuda_launch_v53_warp_softmax_rowgroup_k_direct_prepared_wmma_partial;
              }
              if (tensor_core_pv_v54_staged_k_warp_softmax_rowgroup_direct_prepared_wmma) {
                launch_standalone_direct_prepared =
                    ds4_cuda_launch_v54_staged_k_warp_softmax_rowgroup_direct_prepared_wmma_partial;
              }
              if (tensor_core_pv_v55_dimsplit_qk_warp_softmax_rowgroup_direct_prepared_wmma) {
                launch_standalone_direct_prepared =
              ...
              if (tensor_core_pv_v58b_v_colmajor_b_direct_prepared_wmma) {
                launch_standalone_direct_prepared =
                    ds4_cuda_launch_v58b_v_colmajor_b_direct_prepared_wmma_partial;
              }
              if (tensor_core_pv_v58_kv_colmajor_b_direct_prepared_wmma) {
                launch_standalone_direct_prepared =
                    ds4_cuda_launch_v58_kv_colmajor_b_direct_prepared_wmma_partial;
              }
              if (tensor_core_pv_v59_a_colmajor_direct_prepared_wmma) {
                launch_standalone_direct_prepared =
                    ds4_cuda_launch_v59_a_colmajor_direct_prepared_wmma_partial;
              }
...
#undef DSV4_LAUNCH_DIRECT_PREPARED_KV
              ...
              if (profile_partial_stages) {
                ...
                ds4_cuda_fused_v8_mma_partial_kernel<false, 17, __nv_bfloat16>
                    <<<split_grid, split_block, 0, stream>>>(
                        reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                        swa_k_cache.data_ptr<uint8_t>(),
                        swa_indices.data_ptr<int32_t>(),
            ...
            } else if (tensor_core_pv_prepared_kv_rolling_q) {
              if (profile_partial_stages) {
                ds4_cuda_fused_v8_mma_partial_kernel<false, 18, __nv_bfloat16, true>
                    <<<split_grid, split_block, 0, stream>>>(
                        reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                        swa_k_cache.data_ptr<uint8_t>(),
                        swa_indices.data_ptr<int32_t>(),
                ...
                ds4_cuda_fused_v8_mma_partial_kernel<false, 18, __nv_bfloat16>
                    <<<split_grid, split_block, 0, stream>>>(
                        reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                        swa_k_cache.data_ptr<uint8_t>(),
                        swa_indices.data_ptr<int32_t>(),
            ...
            } else if (tensor_core_pv_prepared_kv_half_pv_warps) {
              if (profile_partial_stages) {
                ds4_cuda_fused_v8_mma_partial_kernel<false, 19, __nv_bfloat16, true>
                    <<<split_grid, v47_block, 0, stream>>>(
                        reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                        swa_k_cache.data_ptr<uint8_t>(),
                        swa_indices.data_ptr<int32_t>(),
                        swa_topk_lengths.data_ptr<int32_t>(),
                ...
                ds4_cuda_fused_v8_mma_partial_kernel<false, 19, __nv_bfloat16>
                    <<<split_grid, v47_block, 0, stream>>>(
                        reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                        swa_k_cache.data_ptr<uint8_t>(),
                        swa_indices.data_ptr<int32_t>(),
              ...
              ds4_cuda_fused_v8_mma_partial_kernel<false, 15, __nv_bfloat16, true>
                  <<<split_grid, split_block, 0, stream>>>(
                      reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                      swa_k_cache.data_ptr<uint8_t>(),
                      swa_indices.data_ptr<int32_t>(),
              ...
              ds4_cuda_fused_v8_mma_partial_kernel<false, 15, __nv_bfloat16>
                  <<<split_grid, split_block, 0, stream>>>(
                      reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                      swa_k_cache.data_ptr<uint8_t>(),
                      swa_indices.data_ptr<int32_t>(),
            ...
            ds4_cuda_fused_v8_mma_partial_kernel<true, 14, __nv_bfloat16, true>
                <<<split_grid, split_block, 0, stream>>>(
                    reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                    swa_k_cache.data_ptr<uint8_t>(),
                    swa_indices.data_ptr<int32_t>(),
            ...
            ds4_cuda_fused_v8_mma_partial_kernel<true, 14, __nv_bfloat16>
                <<<split_grid, split_block, 0, stream>>>(
                    reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                    swa_k_cache.data_ptr<uint8_t>(),
                    swa_indices.data_ptr<int32_t>(),
        ...
        } else if (tensor_core_pv_inline_mma_store) {
          if (profile_partial_stages) {
            ds4_cuda_fused_v8_mma_partial_kernel<true, 11, __nv_bfloat16, true>
                <<<split_grid, split_block, 0, stream>>>(
                    reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                    swa_k_cache.data_ptr<uint8_t>(),
                    swa_indices.data_ptr<int32_t>(),
            ...
            ds4_cuda_fused_v8_mma_partial_kernel<true, 11, __nv_bfloat16>
                <<<split_grid, split_block, 0, stream>>>(
                    reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                    swa_k_cache.data_ptr<uint8_t>(),
                    swa_indices.data_ptr<int32_t>(),
        ...
        } else if (tensor_core_pv_direct_acc_store) {
          if (profile_partial_stages) {
            ds4_cuda_fused_v8_mma_partial_kernel<true, 10, __nv_bfloat16, true>
                <<<split_grid, split_block, 0, stream>>>(
                    reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                    swa_k_cache.data_ptr<uint8_t>(),
                    swa_indices.data_ptr<int32_t>(),
            ...
            ds4_cuda_fused_v8_mma_partial_kernel<true, 10, __nv_bfloat16>
                <<<split_grid, split_block, 0, stream>>>(
                    reinterpret_cast<const __nv_bfloat16*>(q.data_ptr<at::BFloat16>()),
                    swa_k_cache.data_ptr<uint8_t>(),
                    swa_indices.data_ptr<int32_t>(),
...
torch::Tensor ds4_cuda_optimized_v36_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV36);
}
...
torch::Tensor ds4_cuda_optimized_v37_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV37);
}
...
torch::Tensor ds4_cuda_optimized_v38_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV38);
}
...
torch::Tensor ds4_cuda_optimized_v39_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV39);
}
...
torch::Tensor ds4_cuda_optimized_v40_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV40);
}
...
torch::Tensor ds4_cuda_optimized_v41_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV41);
}
...
torch::Tensor ds4_cuda_optimized_v42_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV42);
}
...
torch::Tensor ds4_cuda_optimized_v43_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV43);
}
...
torch::Tensor ds4_cuda_optimized_v44_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV44);
}
...
torch::Tensor ds4_cuda_optimized_v45_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV45);
}
...
torch::Tensor ds4_cuda_optimized_v46_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV46);
}
...
torch::Tensor ds4_cuda_optimized_v47_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV47);
}
...
torch::Tensor ds4_cuda_optimized_v51_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV51);
}
...
torch::Tensor ds4_cuda_optimized_v52_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV52);
}
...
torch::Tensor ds4_cuda_optimized_v53_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV53);
}
...
torch::Tensor ds4_cuda_optimized_v54_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV54);
}
...
torch::Tensor ds4_cuda_optimized_v55_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV55);
}
...
torch::Tensor ds4_cuda_optimized_v56_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV56);
}
...
torch::Tensor ds4_cuda_optimized_v57_attention(
  ...
  return launch_ds4_cuda_attention(
      ...
      extra_topk_lengths,
      extra_page_size,
      AttentionVariant::kOptimizedV57);
}

#define DSV4_DEFINE_OPTIMIZED_WRAPPER(NAME, VARIANT) \
  torch::Tensor NAME(                                \
      torch::Tensor q,                               \
      torch::Tensor swa_k_cache,                     \
      torch::Tensor swa_indices,                     \
...
DSV4_DEFINE_OPTIMIZED_WRAPPER(
    ...
    AttentionVariant::kOptimizedV58A)
DSV4_DEFINE_OPTIMIZED_WRAPPER(
    ds4_cuda_optimized_v58b_attention,
    AttentionVariant::kOptimizedV58B)
DSV4_DEFINE_OPTIMIZED_WRAPPER(
    ds4_cuda_optimized_v58_attention,
    AttentionVariant::kOptimizedV58)
DSV4_DEFINE_OPTIMIZED_WRAPPER(
    ds4_cuda_optimized_v59_attention,
    AttentionVariant::kOptimizedV59)
DSV4_DEFINE_OPTIMIZED_WRAPPER(
